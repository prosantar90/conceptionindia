    ...
    gsap.registerEffect({
      ...
      effect: (target, config) => {
        ...
        const
		  ...
		  $curve = $target.find('.curtain-svg__normal'),
    ...
    gsap.registerEffect({
      ...
      effect: (target, config) => {
        ...
        if (window.innerWidth / window.innerHeight >= 1) {
			$curveTop = $target.find('.curtain-svg__normal');
			$curveBottom = $svg.find('.curtain-svg__normal');
        } else {
			$curveTop = $svg.find('.curtain-svg__normal');
			$curveBottom = $svg.find('.curtain-svg__normal');
        }
    ...
    if ( window.location.hash ) {
      ...
      if ($scrollElement.length) {
        setTimeout(() => {
          Scroll.scrollTo({
            y: 0,
            x: $scrollElement.offset().top - offsetTop,
            duration: 800
          });
      }, timeout);
      }
